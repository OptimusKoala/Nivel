...
La règle « surcharge valide sinon défaut du catalogue » est **une seule fonction publique**, pas une par appelant : `ReminderSchedule.resolvedMinutes(_:for:)` et `ReminderSchedule.resolvedWeekday(_:for:)`. `ReminderPlanner.planned` les appelle, et l'écran de réglage (§4.4) aussi, pour afficher exactement ce que le téléphone va planifier. `resolvedWeekday` court-circuite sur `isWeekdayEditable` : un rappel quotidien ne consulte jamais la surcharge stockée, même si une clé y traîne.
...
- **Largeur mesurée** (harnais hébergeant `SettingsContent` réel, 320 à 402 pt) : sans intervention, le libellé du menu de jour (« sam. ») et le sous-titre de fréquence (« chaque semaine ») se coupent tous deux en deux lignes sur la ligne « Pesée ». Le bloc de texte porte `.fixedSize(horizontal: true, vertical: false)`, le menu de jour porte `.fixedSize()` : posés ensemble, aucun des deux ne se coupe, de 320 à 402 pt. Un seul des deux déplace simplement la coupure sur l'élément resté sans protection. À 320 pt la ligne déborde alors de la carte plutôt que de rentrer ; aucun iPhone livré ne fait 320 pt, et d'autres lignes de cet écran cassent déjà seules à cette largeur.
- Le binding d'heure convertit minutes ↔ `Date` via `DateComponents` sur un calendrier **grégorien fixe** (`Calendar(identifier: .gregorian)`, pas `Calendar.current`), sur un jour de référence arbitraire. C'est de l'arithmétique heure/minute pure, aucune sémantique calendaire réelle n'est nécessaire ; avec le calendrier de l'appareil, un réglage Région non grégorien peut faire échouer la construction de la date et faire retomber silencieusement l'affichage sur l'heure courante. Seuls l'heure et la minute sont lus en retour.
