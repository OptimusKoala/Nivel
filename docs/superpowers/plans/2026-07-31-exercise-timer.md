...
- `SessionPlayerSheet` gagne `@State private var pulsingCTA = false`, mis à `true` quand la page COURANTE signale un timer fini (remonter l'info par closure `onTimerFinished: { if page == pageDeLaStruct { pulsingCTA = true } }` ou binding par page), remis à `false` à chaque changement de page (`.onChange(of: page)`).
