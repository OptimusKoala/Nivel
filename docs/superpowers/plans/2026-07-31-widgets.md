...
Note : `extras` est un tableau de tuples `[(Extra, Int)]` (défaut `[]`), PAS un dictionnaire — reprendre la signature exacte utilisée par `GameServiceTests` si elle diffère.
...
```bash
grep -rn "—" Widgets/ Shared/ App/Services/GameService+WidgetSync.swift App/Services/DeepLink.swift
```

Expected: relire chaque ligne sortie À L'ŒIL — les tirets cadratins dans les commentaires (`//`) sont acceptés, ceux dans une chaîne utilisateur (`Text(...)`, littéraux `"..."` affichés) sont interdits.
